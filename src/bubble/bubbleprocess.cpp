...
vector<bubblePoint> bubbleProcess::convertOmniGrayImage2Bub(cv::Mat grayImage, int maxval)
{
   ...
   // if(imageTiltAngles.size() == 0) calculateImageTiltAngles(focalLengthPixels,grayImage.cols,grayImage.rows);
    ...
    for(int i = 0; i < grayImage.rows; i++)
    {
        ...
        //    qDebug()<<pan<<" "<<tilt;
                ...
                float tilt_fov=43.8;
                float pan = (360*j)/(float)grayImage.cols;
                float tilt = (2*tilt_fov / (float)grayImage.rows)*i-tilt_fov;
...
vector<bubblePoint> bubbleProcess::convertOmniGrayImage2BubCustom(cv::Mat grayImage, int maxval, int lower_cut_size, int upper_cut_size)
{
   ...
   // if(imageTiltAngles.size() == 0) calculateImageTiltAngles(focalLengthPixels,grayImage.cols,grayImage.rows);
    ...
    for(int i = upper_cut_size; i < grayImage.rows-lower_cut_size; i++)
    {
        for(int j = 0; j < grayImage.cols; j++)
        {
...
vector<bubblePoint> bubbleProcess::convertOmniGrayImage2BubFront(cv::Mat grayImage, int maxval)
{

    vector<bubblePoint> result;

    for(int i = 0; i < grayImage.rows-80; i++)
    {
        for(int j = 0; j < grayImage.cols; j++)
        {
...
vector<bubblePoint> bubbleProcess::convertOmniGrayImage2BubFrontCustom(cv::Mat grayImage, int maxval, int lower_cut_size, int upper_cut_size)
{
...
vector<bubblePoint> bubbleProcess::convertOmniGrayImage2BubFrontBar(cv::Mat grayImage, int maxval)
{
...
vector<bubblePoint> bubbleProcess::convertOmniGrayImage2BubFrontBarCustom(cv::Mat grayImage, int maxval, int lower_cut_size, int upper_cut_size)
{
